...
//        // Apply local door transformations
//        float DoorWidth = 0.3f;
//        float DoorHeight = 1.0f;
//        // Adjust the model matrix to position the door correctly on the house
//        model = glm::translate(model, glm::vec3(1.3f, -0.45f + DoorHeight / 2, 0.01f)); // Small z-offset to prevent z-fighting
//        // Scale the door to the correct size
//        model = glm::scale(model, glm::vec3(1.0f, 1.0f, 1.0f)); // Scale to door size
//
//        // Set the transformed model matrix for the door
//        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
//
...
//
//        // Draw door
//        glBindVertexArray(houseVAO); // Make sure to bind the house VAO
//        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, (void*)(36 * sizeof(GLuint))); // 6 indices for the door, starting from index 36
//
